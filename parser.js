var example_text = ". AO1 L | HH Y UW1 M AH0 N | B IY1 IH0 NG Z | AA1 R | B AO1 R N | \
...
function convert(symbol, pre, post, with_stress, cutspell) {
  let stress = symbol.substr(2, 1)
  let symbol_nos = symbol.substr(0, 2)
  let pre_nos = pre.substr(0, 2)
  let post_nos = post.substr(0, 2)
  ...
  switch (symbol_nos) {
    ...
    case 'AO':
      ...
      return pickStress('áa', 'aa')
    case 'AW':
      if (post_nos === 'UH' || post_nos === 'UW') {
        return pickStress('áw', 'aw')
      }
      return pickStress('áu', 'au')
    case 'AY':
      if (post_nos === 'IH' || post_nos === 'IY') {
