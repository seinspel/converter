...
import * as Assemble$Converter from "./Assemble.mjs";
import * as Constants$Converter from "./Constants.mjs";
import * as Controller$Converter from "./Controller.mjs";
import * as AsciiDecompression$Converter from "./AsciiDecompression.mjs";
...
function findBaseForm(chunk) {
  ...
  switch (chunkLast2) {
    ...
    case "LY" :
        var resultLLY = chunkLast3 === "LLY" ? addSuffix(lookup(chunk.slice(0, -2)), "}", ["j"]) : undefined;
...
function processText(param) {
  ...
  var impliedLong = document.getElementById("impliedLong").checked;
  var match;
  if (withMacrons) {
    match = [
      Constants$Converter.lexicalsetsMacron,
      Constants$Converter.consonantsMacron
    ];
  } else if (withStress) {
    match = [
    ...
    var match$1 = Controller$Converter.loadSpelling(undefined);
    match = [
      match$1[0],
      match$1[1]
    ];
