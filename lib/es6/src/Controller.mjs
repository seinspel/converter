// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Constants$Converter from "./Constants.mjs";

function loadSpelling(param) {
  var vowels_kit = [
    "",
    document.getElementById("kit-vowel").value
  ];
  var vowels_dress = [
    "",
    document.getElementById("dress-vowel").value
  ];
  var vowels_trap = [
    "",
    document.getElementById("trap-vowel").value
  ];
  var vowels_lot = [
    "",
    document.getElementById("lot-vowel").value
  ];
  var vowels_strut = [
    "",
    document.getElementById("strut-vowel").value
  ];
  var vowels_foot = [
    "",
    document.getElementById("foot-vowel").value
  ];
  var vowels_cloth = [
    "",
    document.getElementById("cloth-vowel").value
  ];
  var vowels_nurse = [
    "",
    document.getElementById("nurse-vowel").value
  ];
  var vowels_fleece = [
    "",
    document.getElementById("fleece-vowel").value
  ];
  var vowels_face = [
    "",
    document.getElementById("face-vowel").value
  ];
  var vowels_palm = [
    "",
    document.getElementById("palm-vowel").value
  ];
  var vowels_thought = [
    "",
    document.getElementById("thought-vowel").value
  ];
  var vowels_goat = [
    "",
    document.getElementById("goat-vowel").value
  ];
  var vowels_goose = [
    "",
    document.getElementById("goose-vowel").value
  ];
  var vowels_price = [
    "",
    document.getElementById("price-vowel").value
  ];
  var vowels_choice = [
    "",
    document.getElementById("choice-vowel").value
  ];
  var vowels_mouth = [
    "",
    document.getElementById("mouth-vowel").value
  ];
  var vowels_cute = [
    "",
    document.getElementById("cute-vowel").value
  ];
  var vowels_near = [
    "",
    document.getElementById("near-vowel").value
  ];
  var vowels_square = [
    "",
    document.getElementById("square-vowel").value
  ];
  var vowels_start = [
    "",
    document.getElementById("start-vowel").value
  ];
  var vowels_north = [
    "",
    document.getElementById("north-vowel").value
  ];
  var vowels_force = [
    "",
    document.getElementById("force-vowel").value
  ];
  var vowels_poor = [
    "",
    document.getElementById("poor-vowel").value
  ];
  var vowels_cure = [
    "",
    document.getElementById("cure-vowel").value
  ];
  var vowels_fire = [
    "",
    document.getElementById("fire-vowel").value
  ];
  var vowels_flour = [
    "",
    document.getElementById("flour-vowel").value
  ];
  var vowels_coir = [
    "",
    document.getElementById("coir-vowel").value
  ];
  var vowels_ian = [
    "",
    document.getElementById("ian-vowel").value
  ];
  var vowels_happy = document.getElementById("happy-vowel").value;
  var vowels_letter = document.getElementById("letter-vowel").value;
  var vowels_comma = document.getElementById("comma-vowel").value;
  var vowels = {
    kit: vowels_kit,
    dress: vowels_dress,
    trap: vowels_trap,
    lot: vowels_lot,
    strut: vowels_strut,
    foot: vowels_foot,
    cloth: vowels_cloth,
    nurse: vowels_nurse,
    fleece: vowels_fleece,
    face: vowels_face,
    palm: vowels_palm,
    thought: vowels_thought,
    goat: vowels_goat,
    goose: vowels_goose,
    price: vowels_price,
    choice: vowels_choice,
    mouth: vowels_mouth,
    cute: vowels_cute,
    near: vowels_near,
    square: vowels_square,
    start: vowels_start,
    north: vowels_north,
    force: vowels_force,
    poor: vowels_poor,
    cure: vowels_cure,
    fire: vowels_fire,
    flour: vowels_flour,
    coir: vowels_coir,
    ian: vowels_ian,
    happy: vowels_happy,
    letter: vowels_letter,
    comma: vowels_comma,
    seperator: "'"
  };
  var consonants_b = document.getElementById("b-consonant").value;
  var consonants_ch = document.getElementById("ch-consonant").value;
  var consonants_d = document.getElementById("d-consonant").value;
  var consonants_dh = document.getElementById("dh-consonant").value;
  var consonants_el = document.getElementById("el-consonant").value;
  var consonants_em = document.getElementById("em-consonant").value;
  var consonants_en = document.getElementById("en-consonant").value;
  var consonants_er = document.getElementById("er-consonant").value;
  var consonants_f = document.getElementById("f-consonant").value;
  var consonants_g = document.getElementById("g-consonant").value;
  var consonants_hh = document.getElementById("h-consonant").value;
  var consonants_j = document.getElementById("j-consonant").value;
  var consonants_k = document.getElementById("k-consonant").value;
  var consonants_l = document.getElementById("l-consonant").value;
  var consonants_m = document.getElementById("m-consonant").value;
  var consonants_n = document.getElementById("n-consonant").value;
  var consonants_ng = document.getElementById("ng-consonant").value;
  var consonants_p = document.getElementById("p-consonant").value;
  var consonants_crv = document.getElementById("crv-consonant").value;
  var consonants_vrv = document.getElementById("vrv-consonant").value;
  var consonants_cs = document.getElementById("s-consonant-un").value;
  var consonants_vs = document.getElementById("s-consonant-am").value;
  var consonants_sh = document.getElementById("sh-consonant").value;
  var consonants_t = document.getElementById("t-consonant").value;
  var consonants_th = document.getElementById("th-consonant").value;
  var consonants_v = document.getElementById("v-consonant").value;
  var consonants_w = document.getElementById("w-consonant").value;
  var consonants_wh = document.getElementById("wh-consonant").value;
  var consonants_y = document.getElementById("y-consonant").value;
  var consonants_zc = document.getElementById("z-consonant-un").value;
  var consonants_zv = document.getElementById("z-consonant-am").value;
  var consonants_zh = document.getElementById("zh-consonant").value;
  var consonants = {
    b: consonants_b,
    ch: consonants_ch,
    d: consonants_d,
    dh: consonants_dh,
    el: consonants_el,
    em: consonants_em,
    en: consonants_en,
    er: consonants_er,
    f: consonants_f,
    g: consonants_g,
    hh: consonants_hh,
    j: consonants_j,
    k: consonants_k,
    l: consonants_l,
    m: consonants_m,
    n: consonants_n,
    ng: consonants_ng,
    p: consonants_p,
    crv: consonants_crv,
    vrv: consonants_vrv,
    cs: consonants_cs,
    vs: consonants_vs,
    sh: consonants_sh,
    t: consonants_t,
    th: consonants_th,
    v: consonants_v,
    w: consonants_w,
    wh: consonants_wh,
    y: consonants_y,
    zc: consonants_zc,
    zv: consonants_zv,
    zh: consonants_zh
  };
  return [
          vowels,
          consonants
        ];
}

function setPreset(preset) {
  var match;
  switch (preset) {
    case "asian" :
        match = [
          Constants$Converter.lexicalsetsAsian,
          Constants$Converter.consonantsAsian
        ];
        break;
    case "european" :
        match = [
          Constants$Converter.lexicalSetsEuropean,
          Constants$Converter.consonantsEuropean
        ];
        break;
    case "greek" :
        match = [
          Constants$Converter.lexicalsetsGreek,
          Constants$Converter.consonantsGreek
        ];
        break;
    case "ipa" :
        match = [
          Constants$Converter.lexicalsetsIPA,
          Constants$Converter.consonantsIPA
        ];
        break;
    case "russian" :
        match = [
          Constants$Converter.lexicalsetsRussian,
          Constants$Converter.consonantsRussian
        ];
        break;
    default:
      match = [
        Constants$Converter.lexicalsetsEnglish,
        Constants$Converter.consonantsEnglish
      ];
  }
  var vowels = match[0];
  var consonants = match[1];
  var tmp = document.getElementById("kit-vowel");
  tmp.value = vowels.kit[1];
  var tmp$1 = document.getElementById("dress-vowel");
  tmp$1.value = vowels.dress[1];
  var tmp$2 = document.getElementById("trap-vowel");
  tmp$2.value = vowels.trap[1];
  var tmp$3 = document.getElementById("lot-vowel");
  tmp$3.value = vowels.lot[1];
  var tmp$4 = document.getElementById("strut-vowel");
  tmp$4.value = vowels.strut[1];
  var tmp$5 = document.getElementById("foot-vowel");
  tmp$5.value = vowels.foot[1];
  var tmp$6 = document.getElementById("cloth-vowel");
  tmp$6.value = vowels.cloth[1];
  var tmp$7 = document.getElementById("nurse-vowel");
  tmp$7.value = vowels.nurse[1];
  var tmp$8 = document.getElementById("fleece-vowel");
  tmp$8.value = vowels.fleece[1];
  var tmp$9 = document.getElementById("face-vowel");
  tmp$9.value = vowels.face[1];
  var tmp$10 = document.getElementById("palm-vowel");
  tmp$10.value = vowels.palm[1];
  var tmp$11 = document.getElementById("thought-vowel");
  tmp$11.value = vowels.thought[1];
  var tmp$12 = document.getElementById("goat-vowel");
  tmp$12.value = vowels.goat[1];
  var tmp$13 = document.getElementById("goose-vowel");
  tmp$13.value = vowels.goose[1];
  var tmp$14 = document.getElementById("price-vowel");
  tmp$14.value = vowels.price[1];
  var tmp$15 = document.getElementById("choice-vowel");
  tmp$15.value = vowels.choice[1];
  var tmp$16 = document.getElementById("mouth-vowel");
  tmp$16.value = vowels.mouth[1];
  var tmp$17 = document.getElementById("cute-vowel");
  tmp$17.value = vowels.cute[1];
  var tmp$18 = document.getElementById("near-vowel");
  tmp$18.value = vowels.near[1];
  var tmp$19 = document.getElementById("square-vowel");
  tmp$19.value = vowels.square[1];
  var tmp$20 = document.getElementById("start-vowel");
  tmp$20.value = vowels.start[1];
  var tmp$21 = document.getElementById("north-vowel");
  tmp$21.value = vowels.north[1];
  var tmp$22 = document.getElementById("force-vowel");
  tmp$22.value = vowels.force[1];
  var tmp$23 = document.getElementById("poor-vowel");
  tmp$23.value = vowels.poor[1];
  var tmp$24 = document.getElementById("cure-vowel");
  tmp$24.value = vowels.cure[1];
  var tmp$25 = document.getElementById("fire-vowel");
  tmp$25.value = vowels.fire[1];
  var tmp$26 = document.getElementById("flour-vowel");
  tmp$26.value = vowels.flour[1];
  var tmp$27 = document.getElementById("coir-vowel");
  tmp$27.value = vowels.coir[1];
  var tmp$28 = document.getElementById("ian-vowel");
  tmp$28.value = vowels.ian[1];
  var tmp$29 = document.getElementById("happy-vowel");
  tmp$29.value = vowels.happy;
  var tmp$30 = document.getElementById("letter-vowel");
  tmp$30.value = vowels.letter;
  var tmp$31 = document.getElementById("comma-vowel");
  tmp$31.value = vowels.comma;
  var tmp$32 = document.getElementById("b-consonant");
  tmp$32.value = consonants.b;
  var tmp$33 = document.getElementById("ch-consonant");
  tmp$33.value = consonants.ch;
  var tmp$34 = document.getElementById("d-consonant");
  tmp$34.value = consonants.d;
  var tmp$35 = document.getElementById("dh-consonant");
  tmp$35.value = consonants.dh;
  var tmp$36 = document.getElementById("el-consonant");
  tmp$36.value = consonants.el;
  var tmp$37 = document.getElementById("em-consonant");
  tmp$37.value = consonants.em;
  var tmp$38 = document.getElementById("en-consonant");
  tmp$38.value = consonants.en;
  var tmp$39 = document.getElementById("er-consonant");
  tmp$39.value = consonants.er;
  var tmp$40 = document.getElementById("f-consonant");
  tmp$40.value = consonants.f;
  var tmp$41 = document.getElementById("g-consonant");
  tmp$41.value = consonants.g;
  var tmp$42 = document.getElementById("h-consonant");
  tmp$42.value = consonants.hh;
  var tmp$43 = document.getElementById("j-consonant");
  tmp$43.value = consonants.j;
  var tmp$44 = document.getElementById("k-consonant");
  tmp$44.value = consonants.k;
  var tmp$45 = document.getElementById("l-consonant");
  tmp$45.value = consonants.l;
  var tmp$46 = document.getElementById("m-consonant");
  tmp$46.value = consonants.m;
  var tmp$47 = document.getElementById("n-consonant");
  tmp$47.value = consonants.n;
  var tmp$48 = document.getElementById("ng-consonant");
  tmp$48.value = consonants.ng;
  var tmp$49 = document.getElementById("p-consonant");
  tmp$49.value = consonants.p;
  var tmp$50 = document.getElementById("crv-consonant");
  tmp$50.value = consonants.crv;
  var tmp$51 = document.getElementById("vrv-consonant");
  tmp$51.value = consonants.vrv;
  var tmp$52 = document.getElementById("s-consonant-un");
  tmp$52.value = consonants.cs;
  var tmp$53 = document.getElementById("s-consonant-am");
  tmp$53.value = consonants.vs;
  var tmp$54 = document.getElementById("sh-consonant");
  tmp$54.value = consonants.sh;
  var tmp$55 = document.getElementById("t-consonant");
  tmp$55.value = consonants.t;
  var tmp$56 = document.getElementById("th-consonant");
  tmp$56.value = consonants.th;
  var tmp$57 = document.getElementById("v-consonant");
  tmp$57.value = consonants.v;
  var tmp$58 = document.getElementById("w-consonant");
  tmp$58.value = consonants.w;
  var tmp$59 = document.getElementById("wh-consonant");
  tmp$59.value = consonants.wh;
  var tmp$60 = document.getElementById("y-consonant");
  tmp$60.value = consonants.y;
  var tmp$61 = document.getElementById("z-consonant-un");
  tmp$61.value = consonants.zc;
  var tmp$62 = document.getElementById("z-consonant-am");
  tmp$62.value = consonants.zv;
  var tmp$63 = document.getElementById("zh-consonant");
  tmp$63.value = consonants.zh;
}

export {
  loadSpelling ,
  setPreset ,
}
/* No side effect */
